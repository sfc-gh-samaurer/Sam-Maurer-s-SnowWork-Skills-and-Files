...
After generating the diagram files, save them to Google Drive under the account's dedicated folder:
**SnowWork → Accounts → {AccountName} → Architecture**
...
**Steps:**
1. Search for the account folder: `mcp_google-worksp_search_drive` query="{AccountName}"
2. If the `Architecture` subfolder does not exist, create the folder chain:
   - `mcp_google-worksp_create_drive_folder` name="{AccountName}", parent_id=`1Yk76x9n8uyghuBrwt_gPehVG7TH6RNck` → `{account_folder_id}`
   - `mcp_google-worksp_create_drive_folder` name="Architecture", parent_id=`{account_folder_id}` → `{architecture_folder_id}`
3. **PNG upload:** There is no automated script for uploading PNG/SVG files to Drive. After generating the files locally, inform the user of the target folder and provide the Google Drive link so they can manually upload the PNG (and optionally the SVG) to the Architecture folder.

**Note to user:** The generated files are saved locally. Please upload the PNG (and SVG if needed) to your Architecture folder in Google Drive:
`https://drive.google.com/drive/folders/{architecture_folder_id}`

## Bullet Formatting Rule

When passing any content to `mcp_google-worksp_create_document`, use markdown `- item` list syntax for all bullet lists. Never use `• item` (bullet character) in document content.
