#!/usr/bin/env python3
"""
build_pov_html.py - Convert a Services POV markdown report to styled HTML.

Purpose-built for Services Point of View documents (not Account Discovery).
Uses Snowflake brand guidelines (colors, fonts, layout).

Usage:
    python <SKILL_DIR>/scripts/build_pov_html.py \
...
<div class="header">
  <div class="header-inner">
    ...
    <h1>{company}</h1>
    <div class="subtitle">Services Point of View</div>
    <div class="meta">
      <span>{date}</span>
      <span>{industry}</span>
      <span>CONFIDENTIAL</span>
    </div>
    <div class="pipeline-badge">Pipeline Opportunity: {pipeline_value}</div>
  </div>
</div>
...
<div class="footer">
  <span class="sf-logo">SNOWFLAKE</span> &middot; Services Point of View &middot; {date} &middot; Confidential
  <br><em style="font-size:10px; color:#b0b0b0;">Investment estimates are indicative and require Pre-Sales Architecture validation before external use.</em>
</div>
...
def main():
    parser = argparse.ArgumentParser(description="Convert Services POV markdown to styled HTML")
    ...
    dash = r'[\u2013\u2014\-]'
    pipeline_patterns = [
        r'(?:Pipeline Opportunity|Services Opportunity|Total.*Pipeline)[:\s|]*\s*(\$[\d.,]+[A-Za-z]*\s*' + dash + r'\s*\$?[\d.,]+[A-Za-z]*)',
        r'(?:Investment|Estimated Investment)[:\s|]*\s*(\$[\d.,]+[A-Za-z]*\s*' + dash + r'\s*\$?[\d.,]+[A-Za-z]*)',
        r'(?:pipeline EACV|active pipeline)[^$]*?(\$[\d.,]+[A-Za-z]*)',
        r'(\$[\d.,]+[A-Za-z]*\s*' + dash + r'\s*\$[\d.,]+[A-Za-z]*)\s*(?:pipeline|opportunity)',
    ]
    pipeline_match = None
    for pat in pipeline_patterns:
        pipeline_match = re.search(pat, md_text, re.MULTILINE | re.IGNORECASE)
        if pipeline_match:
            break
    pipeline_value = pipeline_match.group(1).strip().strip('|').strip('*').strip() if pipeline_match else "TBD"
    ...
    result = HTML_TEMPLATE.format(
        title=f"{company} - Services Point of View",
        company=company,
        date=date_str,
        industry=industry,
