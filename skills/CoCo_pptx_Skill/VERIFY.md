...
def verify_slide(slide, prs, slide_num):
    ...
    # --- Check 3f4: Dark-background layout with non-WHITE custom shape text ---
    ...
    if layout_idx in DARK_BG_LAYOUTS or layout_idx in COVER_LAYOUTS:
        for shape in slide.shapes:
            if shape.is_placeholder or not shape.has_text_frame:
                continue
            l_pos = (shape.left or 0) / 914400
            if layout_idx == 9 and l_pos < 2.51:
                continue
            if layout_idx == 11 and l_pos < 5.00:
                continue
            for para in shape.text_frame.paragraphs:
                if not para.text.strip():
                    continue
                for run in para.runs:
                    try:
                        ...
                        if c and c.type is not None and c.rgb:
                            rgb_t = (c.rgb[0], c.rgb[1], c.rgb[2])
                            if rgb_t == (0x26, 0x26, 0x26):
                                issues.append(f"  DARK-BG INVISIBLE: DK1 text \"{run.text[:30]}\" on layout {layout_idx} (dark background) — use WHITE")
                            elif rgb_t not in {(0xFF, 0xFF, 0xFF), (0x29, 0xB5, 0xE8)} and rgb_t[0] + rgb_t[1] + rgb_t[2] < 384:
                                issues.append(f"  DARK-BG LOW CONTRAST: #{c.rgb} text \"{run.text[:30]}\" on layout {layout_idx} — use WHITE for readability")
                    except (AttributeError, TypeError):
                        pass
...
def verify_deck(prs):
    ...
    # Rule 28: Visual variety
    if content_slides > 6:
        required = content_slides // 3
        if visual_pattern_slides < required:
            issues.append(f"  VISUAL MONOTONY: Only {visual_pattern_slides}/{content_slides} content slides use visual patterns (need ≥{required} = 1 in 3)")
    if max_consecutive_text > 3:
        issues.append(f"  CONSECUTIVE TEXT: {max_consecutive_text} text-only slides in a row (max 3)")
...
| VISUAL MONOTONY | Less than 1 in 3 content slides use visual patterns | Add more Section 14 patterns |
| CONSECUTIVE TEXT | More than 3 text-only slides in a row | Insert a visual pattern slide to break monotony |
| DARK-BG INVISIBLE | DK1 (#262626) text on a dark-background layout (9,10,18-22,23,25-28,13-17) | Use WHITE for all custom shape text on dark layouts. `add_shape_text()` auto-corrects this. |
| DARK-BG LOW CONTRAST | Dark text colour on a dark-background layout | Use WHITE (#FFFFFF) for text on layouts with DK2/SF_BLUE/accent1 backgrounds |
