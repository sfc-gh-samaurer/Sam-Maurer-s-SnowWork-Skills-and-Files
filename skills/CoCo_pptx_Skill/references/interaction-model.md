...
Generate the Python code and produce the deck. By default, save to
`~/Google Drive/My Drive/` for automatic sync to Google Drive (opens natively
in Google Slides). Falls back to `~/Downloads/` if Google Drive Desktop is
not installed. See `references/core-helpers.md` Section 18.

After delivery, always offer:

```
Deck is ready! Saved to Google Drive — open it in Google Slides from drive.google.com.
...
- Save a copy to a different location (local or Drive subfolder)?
```
