...
**SnowWork → Accounts → {AccountName} → Bronze Layer**
...
2. If not found, create it:
   ...
   ```
3. Check if `Bronze Layer` subfolder exists under `{AccountName}` folder; create if missing:
   ```
   mcp_google-worksp_create_drive_folder: name="Bronze Layer", parent_id="{AccountName_folder_id}"
   ```
...
./node /Users/michaelkelly/CoCo/Scripts/upload_to_gsheets.mjs \
  "<customer_directory>/<Customer>_Bronze_Layer_Implementation.xlsx" \
  "{Customer} Bronze Layer Implementation" \
  "{Bronze_Layer_folder_id}"
```
