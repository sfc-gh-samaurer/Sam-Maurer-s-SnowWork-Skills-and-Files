...
BEGIN
  ...
  SELECT
      ...
      CAST(ucf.SALESFORCE_USE_CASE_ESTIMATED_ANNUAL_CREDIT_CONSUMPTION AS FLOAT) AS ESTIMATED_ACV,
  ...
  -- ACV/credit-consumption measure was removed from the DD_ (dimension) view upstream;
  -- it now lives in the FCT_ (fact) view. Join it in, keyed by use case id (verified 1:1).
  LEFT JOIN SNOW_CERTIFIED.SALESFORCE_USE_CASE.FCT_SALESFORCE_USE_CASE ucf
      ON m.SALESFORCE_USE_CASE_ID = ucf.SALESFORCE_USE_CASE_ID
