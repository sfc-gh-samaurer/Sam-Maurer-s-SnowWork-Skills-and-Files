...
-- ---------------------------------------------------------------------------
-- Recreate with the full schema (includes STATUS / LAST_ERROR columns). This table
-- is pure bookkeeping — it is fully repopulated by the CALL at the end of this
-- script — so CREATE OR REPLACE is safe and avoids ALTER-compatibility issues on
-- deployments that predate the STATUS columns.
CREATE OR REPLACE TABLE SD_APPS_DB.SD_CENTER.SD_CACHE_METADATA (
    TABLE_NAME        VARCHAR        NOT NULL,
    LAST_REFRESHED_AT TIMESTAMP_LTZ,
    ROW_COUNT         INT,
    STATUS            VARCHAR,          -- 'OK' or 'FAILED' (per-table, set each run)
    LAST_ERROR        VARCHAR,          -- SQLERRM from the most recent failure
    LAST_ERROR_AT     TIMESTAMP_LTZ,    -- when that failure occurred
    PRIMARY KEY (TABLE_NAME)
);

-- ---------------------------------------------------------------------------
-- 2. Stored procedure: refreshes all 15 cache tables with PER-TABLE isolation.
--    Each table rebuild runs in its own BEGIN/EXCEPTION block, so a failure in
--    one source no longer aborts the other 14 (previously the whole dashboard
--    froze on any single upstream error). Per-table STATUS/LAST_ERROR is written
--    to SD_CACHE_METADATA, and a failed table keeps its OLD LAST_REFRESHED_AT so
--    the app can detect staleness. If any table failed, the proc RAISEs at the
--    end so the scheduled Task run is marked FAILED (enabling task alerting).
...
DECLARE
  ...
  cur_tbl STRING DEFAULT '';
  cache_refresh_failed EXCEPTION (-20501, 'One or more SD cache tables failed to refresh; see SD_CACHE_METADATA (STATUS=FAILED, LAST_ERROR).');
BEGIN

  -- ── 1. SD_CACHE_ACCOUNTS_BASE ─────────────────────────────────────────────
  cur_tbl := 'SD_CACHE_ACCOUNTS_BASE';
  BEGIN
  ...
  WHEN MATCHED THEN UPDATE SET LAST_REFRESHED_AT = s.TS, ROW_COUNT = s.CNT, STATUS = 'OK', LAST_ERROR = NULL, LAST_ERROR_AT = NULL
  WHEN NOT MATCHED THEN INSERT (TABLE_NAME, LAST_REFRESHED_AT, ROW_COUNT, STATUS) VALUES (s.TABLE_NAME, s.TS, s.CNT, 'OK');
  EXCEPTION WHEN OTHER THEN
    failed_tables := failed_tables || cur_tbl || '; ';
    MERGE INTO SD_APPS_DB.SD_CENTER.SD_CACHE_METADATA t
    USING (SELECT :cur_tbl AS TABLE_NAME) s
    ON t.TABLE_NAME = s.TABLE_NAME
    WHEN MATCHED THEN UPDATE SET STATUS = 'FAILED', LAST_ERROR = :SQLERRM, LAST_ERROR_AT = CURRENT_TIMESTAMP()
    WHEN NOT MATCHED THEN INSERT (TABLE_NAME, STATUS, LAST_ERROR, LAST_ERROR_AT) VALUES (:cur_tbl, 'FAILED', :SQLERRM, CURRENT_TIMESTAMP());
  END;

  -- ── 2. SD_CACHE_CAPACITY_RENEWALS — REMOVED ──────────────────────────────────
  -- This table was removed from the scheduled refresh. RAVEN views
  -- (DIM_CONTRACT_VIEW, A360_OVERAGE_UNDERAGE_PREDICTION_VIEW) use
  -- CURRENT_USER()-based row-access policies. In task execution context
  -- CURRENT_USER()='SYSTEM' has no territory mapping → empty results.
  -- load_capacity_renewals() in data.py now queries RAVEN directly in the
  -- app's user session (CURRENT_USER()=SAMAURER) where RLS resolves correctly.
  -- @st.cache_data(ttl=86400) handles caching per scope key.

  -- ── 3. SD_CACHE_CAPACITY_PIPELINE ─────────────────────────────────────────
  cur_tbl := 'SD_CACHE_CAPACITY_PIPELINE';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_PS_PROJECTS_ACTIVE';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_ACTION_PLANNER';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_EXEC_SW_RENEWALS';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_EXEC_SVC_RENEWALS';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_EXEC_NEW_OPPS';
  BEGIN
  ...
  cur_tbl := 'SD_CACHE_MILESTONE_ACV';
  BEGIN
  CREATE OR REPLACE TABLE SD_APPS_DB.SD_CENTER.SD_CACHE_MILESTONE_ACV AS
  -- Rewritten to use FIVETRAN/SNOWHOUSE directly (no SNOW_CERTIFIED dependency).
  -- SNOW_CERTIFIED applied row-level security scoped to the TAM role's district
  -- access, causing intermittent auth failures and incomplete global coverage.
  SELECT
      m.NAME                                                  AS MILESTONE_NAME,
      m.PSE_STATUS_C                                          AS MILESTONE_STATUS,
      m.ID                                                    AS MILESTONE_ID,
      p.NAME                                                  AS PROJECT_NAME,
      p.PSE_STAGE_C                                          AS PROJECT_STAGE,
      p.PSE_PROJECT_STATUS_C                                  AS PROJECT_STATUS,
      p.PSE_START_DATE_C                                      AS PROJECT_START,
      p.PSE_END_DATE_C                                        AS PROJECT_END,
      a.ACCOUNT_NAME,
      a.ACCOUNT_ID                                            AS SALESFORCE_ACCOUNT_ID,
      ...
      CAST(uc.ESTIMATED_ANNUAL_CREDIT_CONSUMPTION_C AS FLOAT) AS ESTIMATED_ACV,
      fc.FISCAL_QUARTER_FYYYYY_QQ                             AS FISCAL_QUARTER_LABEL
  FROM FIVETRAN.SALESFORCE.PSE_MILESTONE_C m
  JOIN FIVETRAN.SALESFORCE.PSE_PROJ_C p
      ON m.PSE_PROJECT_C = p.ID
      AND p.IS_DELETED = FALSE
  JOIN SNOWHOUSE.SALES.ACCOUNTS_DAILY a
      ON p.PSE_ACCOUNT_C = a.ACCOUNT_ID
      AND a.DS = (SELECT MAX(DS) FROM SNOWHOUSE.SALES.ACCOUNTS_DAILY)
  LEFT JOIN FIVETRAN.SALESFORCE.USE_CASE_C uc
      ON m.USE_CASE_C = uc.ID
      AND uc._FIVETRAN_DELETED = FALSE
  LEFT JOIN SNOWHOUSE.UTILS.FISCAL_CALENDAR fc
      ON fc._DATE = uc.ACTUAL_GO_LIVE_DATE_C
  WHERE m.MILESTONE_TYPE_C = 'Use Case'
  AND m._FIVETRAN_DELETED = FALSE
  AND uc.ACTUAL_GO_LIVE_DATE_C IS NOT NULL
  ORDER BY uc.ACTUAL_GO_LIVE_DATE_C;
  ...
  cur_tbl := 'SD_CACHE_WOW_USE_CASES';
  BEGIN
  ...
  -- If any table failed, surface it: RAISE so the scheduled Task run is marked
  -- FAILED (this is what drives task-level alerting + SUSPEND_TASK_AFTER_NUM_FAILURES).
...
-- Verify: check that all 15 tables are populated, recent, and STATUS = 'OK'.
-- Any row with STATUS = 'FAILED' shows which source broke and the error text.
SELECT TABLE_NAME, LAST_REFRESHED_AT, ROW_COUNT, STATUS, LAST_ERROR, LAST_ERROR_AT
