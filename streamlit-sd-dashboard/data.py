    ...
    df = session.sql("""
        ...
        capacity AS (
            SELECT
                ...
                MIN(CASE WHEN a3.DAY_OF_OVERAGE >= CURRENT_DATE THEN a3.DAY_OF_OVERAGE END) AS OVERAGE_DATE
            FROM SALES.RAVEN.DD_SALESFORCE_ACCOUNT_A360 a3
            WHERE a3.SALESFORCE_ACCOUNT_ID IN (SELECT SALESFORCE_ACCOUNT_ID FROM base)
            AND a3.CAPACITY_PURCHASED > 0
            GROUP BY a3.SALESFORCE_ACCOUNT_ID
        ),
        bob AS (
            SELECT SALESFORCE_ACCOUNT_ID,
                   CAST(SUM(CAPACITY_PURCHASED) AS FLOAT) AS CAPACITY_PURCHASED,
                   CAST(SUM(TOTAL_CAPACITY) AS FLOAT) AS TOTAL_CAPACITY,
                   CAST(SUM(CAPACITY_USED) AS FLOAT) AS CAPACITY_USED,
                   CAST(SUM(CAPACITY_REMAINING) AS FLOAT) AS CAPACITY_REMAINING,
                   MIN(CONTRACT_START_DATE) AS CONTRACT_START_DATE,
                   MAX(CONTRACT_END_DATE) AS CONTRACT_END_DATE
            FROM (
                SELECT *, ROW_NUMBER() OVER (PARTITION BY SALESFORCE_ACCOUNT_ID, SALESFORCE_CONTRACT_ID ORDER BY LOADED_AT DESC) AS rn
                FROM SALES.RAVEN.BOB_CONTRACTS
                WHERE SALESFORCE_ACCOUNT_ID IN (
                    SELECT SALESFORCE_ACCOUNT_ID FROM base
                    WHERE SALESFORCE_ACCOUNT_ID NOT IN (SELECT SALESFORCE_ACCOUNT_ID FROM capacity)
                )
            ) t
            WHERE rn = 1
            GROUP BY SALESFORCE_ACCOUNT_ID
        ),
        contract_fallback AS (
            SELECT
            ...
            AND c.ACCOUNT_ID IN (
                SELECT SALESFORCE_ACCOUNT_ID FROM base
                WHERE SALESFORCE_ACCOUNT_ID NOT IN (SELECT SALESFORCE_ACCOUNT_ID FROM capacity)
                AND SALESFORCE_ACCOUNT_ID NOT IN (SELECT SALESFORCE_ACCOUNT_ID FROM bob)
            )
        ...
        SELECT
            ...
            b.TIER,
            b.LEAD_SE,
            COALESCE(cap.CONTRACT_START_DATE, bob.CONTRACT_START_DATE, cf.CONTRACT_START_DATE) AS CONTRACT_START_DATE,
            COALESCE(cap.CONTRACT_END_DATE, bob.CONTRACT_END_DATE, cf.CONTRACT_END_DATE) AS CONTRACT_END_DATE,
            COALESCE(cap.CAPACITY_PURCHASED, bob.CAPACITY_PURCHASED) AS CAPACITY_PURCHASED,
            COALESCE(cap.TOTAL_CAPACITY, bob.TOTAL_CAPACITY) AS TOTAL_CAPACITY,
            COALESCE(cap.CAPACITY_USED, bob.CAPACITY_USED) AS CAPACITY_USED,
            COALESCE(cap.CAPACITY_REMAINING, bob.CAPACITY_REMAINING) AS CAPACITY_REMAINING,
        ...
        LEFT JOIN bob ON b.SALESFORCE_ACCOUNT_ID = bob.SALESFORCE_ACCOUNT_ID
        LEFT JOIN contract_fallback cf ON b.SALESFORCE_ACCOUNT_ID = cf.SALESFORCE_ACCOUNT_ID
