...
def render_product_expanders(sfdc_acct_ids, product_data, section_key):
    ...
    summary = acct_products.groupby(["ACCOUNT_NAME", "PRODUCT_CATEGORY"]).agg(
        ...
        FEATURES=("PRIMARY_FEATURE", lambda x: ", ".join(sorted(v for v in x.dropna().unique() if isinstance(v, str))))
...
st.markdown('<div class="tab-banner"><p class="tab-banner-title">Active SD Projects</p></div>', unsafe_allow_html=True)
...
st.markdown('<div class="tab-banner"><p class="tab-banner-title">Expiring SD Projects &#8212; No Extension</p></div>', unsafe_allow_html=True)
...
st.markdown('<div class="tab-banner"><p class="tab-banner-title">SD Pipeline (Open Opportunities)</p></div>', unsafe_allow_html=True)
...
st.markdown('<div class="tab-banner"><p class="tab-banner-title">Historical Sold Services &amp; Training</p></div>', unsafe_allow_html=True)
