...
hub = slide.shapes.add_shape(
    MSO_SHAPE.OVAL,
    Inches(hub_cx - hub_r), Inches(hub_cy - hub_r),
    Inches(hub_r * 2), Inches(hub_r * 2))
hub.fill.solid(); hub.fill.fore_color.rgb = DK2
hub.line.fill.background()
tf = hub.text_frame; tf.word_wrap = True
tf.auto_size = MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE  # ← MANDATORY safety net
tf.vertical_anchor = MSO_ANCHOR.MIDDLE
p = tf.paragraphs[0]; p.text = hub_label
p.font.size = Pt(11); p.font.bold = True
p.font.color.rgb = WHITE; p.font.name = "Arial"
p.alignment = PP_ALIGN.CENTER
...
for i, (label, color) in enumerate(spokes):
    ...
    # Spoke circle
    spoke = slide.shapes.add_shape(
        MSO_SHAPE.OVAL,
        Inches(sx - spoke_r), Inches(sy - spoke_r),
        Inches(spoke_r * 2), Inches(spoke_r * 2))
    spoke.fill.solid(); spoke.fill.fore_color.rgb = color
    spoke.line.fill.background()
    tf = spoke.text_frame; tf.word_wrap = True
    tf.auto_size = MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE  # ← MANDATORY safety net
    tf.vertical_anchor = MSO_ANCHOR.MIDDLE
    p = tf.paragraphs[0]; p.text = label
    p.font.size = Pt(9); p.font.bold = True
