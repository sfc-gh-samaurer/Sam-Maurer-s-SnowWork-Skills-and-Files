...
if [[ -f "$SD_TAG_SUBMITTED" ]]; then
    if [[ -f "$SD_TAG_VALUES" ]] && grep -q "^PROJECT=.\+" "$SD_TAG_VALUES" 2>/dev/null; then
        ...
        QUERY_TAG="{\"app\":\"${APP:-snowwork}\",\"customer\":\"${CUSTOMER}\",\"project_id\":\"${PROJECT_ID}\",\"project\":\"${PROJECT}\",\"milestone_id\":\"${MILESTONE_ID}\",\"milestone\":\"${MILESTONE}\",\"email\":\"${EMAIL}\",\"session_id\":\"${SESSION_ID}\"}"
        QUERY_TAG="${QUERY_TAG//\'/\'\'}"
        ...
        if [[ "$PROJECT" == "UNTAGGED" ]]; then
            ...
            if [[ $((REMIND_COUNT % 20)) -eq 0 ]]; then
                echo "IMPORTANT: You MUST display this warning to the user before answering UNLESS the user's prompt is invoking /sd-submit-info (in which case skip the warning and proceed with the skill): This session is still tagged as PROJECT=UNTAGGED. Run /sd-submit-info to tag your actual project and milestone."
            fi
        fi
        exit 0
    fi
...
if [[ "$BLOCK_COUNT" -lt 5 ]]; then
    ...
    echo "IMPORTANT: You MUST display this warning to the user before answering UNLESS the user's prompt is invoking /sd-submit-info (in which case skip the warning and proceed with the skill): This session is not tagged for SD project tracking. Run /sd-submit-info to tag it. It will auto-tag as UNTAGGED in ${REMAINING} prompt(s)."
    exit 0
fi
...
if [[ "$BLOCK_COUNT" -ge 5 ]]; then
    ...
    for SEL in "$CX_BASE/.last_selection_snowwork" "$CX_BASE/.last_selection_cli"; do
...
EOF
    ...
    QUERY_TAG="{\"app\":\"snowwork\",\"customer\":\"UNKNOWN\",\"project_id\":\"000\",\"project\":\"UNTAGGED\",\"milestone_id\":\"000\",\"milestone\":\"UNTAGGED\",\"email\":\"${EMAIL}\",\"session_id\":\"${SESSION_ID}\"}"
    ...
    if [[ $TAG_RC -eq 0 ]]; then
        echo "$(date +%s)|${SESSION_ID}|TAG_AUTO|snowwork|UNTAGGED/${EMAIL}|conn=${TAG_CONN:-none}(${CONN_SOURCE})" >> "$TAG_LOG"
    else
        echo "$(date +%s)|${SESSION_ID}|TAG_FAIL|snowwork|auto_tag_rc=${TAG_RC}|conn=${TAG_CONN:-none}(${CONN_SOURCE})" >> "$TAG_LOG"
    fi
    touch "$SD_TAG_SUBMITTED"
    echo "IMPORTANT: You MUST display this warning to the user before answering UNLESS the user's prompt is invoking /sd-submit-info (in which case skip the warning and proceed with the skill): Session auto-tagged as PROJECT=UNTAGGED. Run /sd-submit-info now to tag your actual project and milestone."
fi
